...
export default async function PropertyPage({
  ...
  const { id } = await params;
  const { property, error } = await getPublicPropertyById(id);
    ...
    return (
      <div className="container-page py-16">
        <EmptyState
          ...
          message={error === "config" ? tCommon("fetchErrorConfig") : tCommon("fetchErrorNetwork")}
        />
      </div>
    );
  }
          ...
          <section className="mt-8">
            ...
            <dl className="mt-3 grid grid-cols-2 gap-y-2 text-sm sm:grid-cols-3">
              <div>
                ...
                <dd className="font-medium text-primary-800 dark:text-white">{property.id}</dd>
              </div>
              <div>
